 ...
 */
@Component("com.lud.core.service.data.builder.types.for.mysql")
public class SQLBuilderServiceForMySQL extends SQLBuilderServiceBase {
    /**
     * 
     * @author sunqinqiu 
    ...
    public String getPagerSQL(SQLRunner runner, DataPager pager) {
        ...
        runner.addArg(pager.getPageSize());
        return this.getSQLBuilder(runner).select().field().from().where().group().order().limit().append(" ?,? ").toString();
    }
    
    /**
     * 获取QuerySQL语句
     * @author sunqinqiu 
     * @date   2019-01-02 22:56
     * @param runner
     * @return
     */
    @Override
    public String getQuerySQL(SQLRunner runner) {
        return this.getSQLBuilder(runner).select().field().from().where().group().order().topLimit().toString();
    }
}
